...
summary_observed <- function(violation,
  ...
  if (include_props_na == TRUE) {

    missing_values_strata <- obs_data %>%
      dplyr::filter(dplyr::if_any(all_of(strata_vars), is.na)) %>%
      dplyr::select(all_of(strata_vars)) %>%
      dplyr::distinct()

    if (nrow(missing_values_strata) == 0) {

      stop("This variable specified in 'strata_vars' does not have missing values (NA's).
           Please change this argument to = FALSE")
    }
  }
